...
export function formationPoint(f,slot){
 let x=0,y=0;
 if(f.kind==='artillery'&&slot>0){x=(slot%2?1:-1)*180;y=(Math.floor((slot-1)/2)-2)*78;}
...
export class Battle{
 constructor(seed=1,{autopilot=false,legacyRoster=false}={}){
  this.version=24;this.launchQueue=[];this.chargeAt=null;this.seed=seed>>>0;this.initialSeed=this.seed;this.time=0;this.tick=0;this.ships=[];this.formations=[];this.bolts=[];this.rockets=[];this.events=[];this.lives=3;this.score=0;this.kills=0;this.hits=0;this.shots=0;this.serial=0;this.projectileSerial=0;this.result=null;this.respawn=0;this.rocketCooldown=0;this.orderNumber=0;this.nextRecall=RULES.firstRecall;this.phase='opening';this.autopilot=autopilot;this.decisionEpoch=0;this.decisionPending=false;this.inference={real:0,fixture:0,fallback:0,status:'standing orders'};this.contacts=[[],[]];this.lastDamage=0;
  for(let side=0;side<2;side++)for(let n=0;n<10;n++){
   ...
   const f={id:side*10+n,side,n,kind,name:(side?'RED ':'')+NAMES[n],x,y,a:dir*Math.PI/2,count:0,leader:null,state:kind==='scout'?'patrol':'travel',target:(1-side)*10+n,goal:{x,y:WORLD.height/2},recall:null,drainAt:0,role:'opening',patrolLeg:side?2:0};
  ...
  for(let side=0;side<2;side++)for(let offset=0;offset<4;offset++){
   ...
   const f={id,side,n,kind:'cargo',name:(side?'RED ':'')+CARGO_NAMES[offset],x:start.x,y:start.y,a:dir*Math.PI/2,count:0,leader:null,state:'convoy',target:null,goal:{...waypoints[1]},recall:null,drainAt:0,role:'civilian',patrolLeg:0,waypoints,routeIndex:1};
 ...
 addCavalryWings(){
  for(let side=0;side<2;side++){
   ...
   const f={id,side,n:14,kind:'cavalry',name:(side?'RED ':'')+'LANCER II',x,y,a:dir*Math.PI/2,count:0,leader:null,state:'travel',target:28+(1-side),goal:{x,y:WORLD.height/2},recall:null,drainAt:0,role:'opening',patrolLeg:0};
   this.formations.push(f);
 ...
 queueReinforcements(){
  for(const hull of this.ships)if(hull.alive&&hull.kind==='artillery'&&hull.lastLaunchEpoch!==this.decisionEpoch){
   hull.lastLaunchEpoch=this.decisionEpoch;
   for(let index=0;index<RULES.fightersPerExchange;index++)this.launchQueue.push({hull:hull.id,epoch:this.decisionEpoch,index,at:this.time+index*RULES.launchSpacing});
  }
 }
 ...
 beginRecall(){
  if(this.phase==='opening')this.phase='combat';
  this.orderNumber++;this.decisionEpoch++;this.decisionPending=true;this.nextRecall=RULES.firstRecall+this.orderNumber*RULES.cycle;
  for(const f of this.formations){if(f.kind!=='fighter'||!f.count)continue;if(this.phase==='final'){f.state='charge';f.recall=null;f.role='attack';f.decision=null;continue;}f.state='regroup';f.recall={x:clamp(f.cx,650,WORLD.width-650),y:clamp(f.cy+(f.side?1:-1)*180,650,WORLD.width-650)};f.drainAt=this.time+RULES.regroup;f.role='attack';f.decision=null;}
  this.queueReinforcements();
  this.events.push({type:'recall',epoch:this.decisionEpoch});this.inference.status='requesting orders';
 }
 // One fresh, team-observed snapshot per side. No hidden contacts or transcript.
 decisionSnapshot(side){
  const visible=new Set(this.contacts[side]),units=[];
  for(const f of this.formations){const m=this.members(f.id).filter(s=>visible.has(s.id));if(!m.length)continue;
   units.push({id:f.id,side:f.side,kind:f.kind,count:m.length,x:Math.round(m.reduce((n,s)=>n+s.x,0)/m.length),y:Math.round(m.reduce((n,s)=>n+s.y,0)/m.length),hull:Math.round(m.reduce((n,s)=>n+s.hp,0)),turrets:m.reduce((n,s)=>n+(s.turrets?.length||0),0)});
  }
  const own=units.filter(f=>f.side===side&&f.kind==='fighter');
  const legal=own.map(f=>{const form=this.formations[f.id],attack=form.role==='attack',targets=units.filter(t=>(attack?t.side!==side:t.side===side&&t.id!==f.id)&&t.kind!=='cargo');return{unit:f.id,role:form.role,choices:[...targets.map(t=>({id:`${attack?'attack':'protect'}:${t.id}`,target:t.id})),{id:'sweep',x:WORLD.width/2,y:WORLD.height/2}]};});
  return{version:1,epoch:this.decisionEpoch,side,time:Math.round(this.time),phase:this.phase,units,legal};
 }
 fallback(snapshot){return snapshot.legal.map(u=>{const own=snapshot.units.find(x=>x.id===u.unit);const opts=u.choices.filter(c=>c.target!==undefined).map(c=>({c,t:snapshot.units.find(t=>t.id===c.target)}));opts.sort((a,b)=>((a.t.kind==='artillery'?0:8000)+Math.hypot(a.t.x-own.x,a.t.y-own.y)+a.t.hull*.05)-((b.t.kind==='artillery'?0:8000)+Math.hypot(b.t.x-own.x,b.t.y-own.y)+b.t.hull*.05));return{unit:u.unit,choice:opts.length?opts[(this.orderNumber+u.unit)%Math.min(2,opts.length)].c.id:'sweep'};});}
 acceptOrders(snapshot,orders,source='inference'){
  if(this.result||snapshot.epoch!==this.decisionEpoch||this.time>RULES.firstRecall+(this.orderNumber-1)*RULES.cycle+RULES.orderWindow||!Array.isArray(orders)||orders.length!==snapshot.legal.length)return false;
  const seen=new Set();for(const o of orders){const l=snapshot.legal.find(u=>u.unit===o.unit);if(!l||seen.has(o.unit)||!l.choices.some(c=>c.id===o.choice))return false;seen.add(o.unit);}
  for(const o of orders){const f=this.formations[o.unit],choice=snapshot.legal.find(u=>u.unit===o.unit).choices.find(c=>c.id===o.choice);f.decision={...choice};if(choice.target!==undefined){const t=snapshot.units.find(x=>x.id===choice.target);f.goal={x:t.x,y:t.y};f.target=t.id;}else {f.goal={x:WORLD.width/2,y:WORLD.height/2};f.target=null;}}
  const counter=source==='inference'?'real':source==='fixture'?'fixture':'fallback';this.inference[counter]++;this.inference.status=source==='inference'?'Luna orders received':'standing orders · signal delayed';return true;
 }
 setFallback(){for(let side=0;side<2;side++){const snap=this.decisionSnapshot(side);if(snap.legal.some(u=>!this.formations[u.unit].decision))this.acceptOrders(snap,this.fallback(snap),'fallback');}this.decisionPending=false;}
 fire(s,a,{turret=null,damage=null,speed=RULES.boltSpeed,light=false,burstFollowup=false}={}){
  if(!s.alive||this.result)return;const gun=turret||s;if(gun.cooldown>0&&!burstFollowup)return;
  if(!burstFollowup)gun.cooldown=turret?(turret.id===7?(turret.burst>0?.3:4.8):.95):TYPES[s.kind].reload;
 ...
 damage(s,shot,amount=1,component=null){
  ...
  if(s.kind!=='artillery'&&!shot.rail&&s.shield>0){s.shield--;this.events.push({type:'shield',x:s.x,y:s.y,side:s.side,player:s.player,popped:s.shield===0,fromPlayer:shot.player,points:hitPoints});return;}
  s.hp=Math.max(0,s.hp-amount);this.events.push({type:'hit',x:s.x,y:s.y,side:s.side,player:s.player,fromPlayer:shot.player,large:s.kind==='artillery',points:hitPoints});
  if(s.hp<=0)this.destroy(s,shot);
 }
 destroy(s,shot={}){
  if(!s.alive||this.result)return;s.alive=false;s.hp=0;let points=0;if(s.side===1){if(shot.player){this.kills++;points=s.kind==='artillery'?10000:s.kind==='cargo'?3500:s.kind==='cavalry'?1500:1000;}else if(s.tagged)points=250;this.score+=points;}this.events.push({type:'explosion',x:s.x,y:s.y,side:s.side,player:s.player,fromPlayer:shot.player,large:s.kind==='artillery'||s.kind==='cargo',targetKind:s.kind,points,assist:points===250});
 ...
 finish(result,reason=''){if(this.result)return;this.result=result;this.reason=reason;this.decisionPending=false;if(result==='won')this.score+=this.lives*2500;this.events.push({type:'end',result});}
 targetFor(s,grid){
 ...
 moveUnit(f,dt){
  ...
  if(f.kind==='cargo'){
   // Both classes share one rendezvous clock, closing their wide lanes together.
   f.goal=frontPoint('cargo',f.n,f.side,this.time);
  ...
  if(this.closing){f.recall=null;f.objective=null;f.state='charge';f.goal={x:WORLD.width/2,y:WORLD.height/2};return;}
  if(this.fortressless){f.objective=null;f.target=null;f.goal={x:WORLD.width/2,y:WORLD.height/2};if(f.kind==='scout')return;}
  if(f.kind==='scout')return;
  if(f.state==='regroup'){
 ...
 npc(s,dt,grid){
  const f=this.formations[s.formation],stats=TYPES[s.kind],awareness=s.kind==='scout'?TYPES.scout.vision:1500;
  ...
  const recalling=f.kind==='fighter'&&f.recall&&(f.state==='regroup'||this.time<f.drainAt+(s.slot%5)*.05);
  if(recalling){
   ...
   let p=legs[f.patrolLeg%4];if(distance(s,p)<250)f.patrolLeg=(f.patrolLeg+1)%4;
   x=p.x;y=p.y;if(this.phase==='final'&&this.time>720){x=WORLD.width/2+Math.sin(this.time/18+s.phase)*3000;y=WORLD.height/2+Math.cos(this.time/21+s.phase)*3000;}
   ...
   if(target&&distance(target,p)<1000){x=target.x;y=target.y;speed=stats.speed;}
   ...
   if(pod){const hold=figureEight(pod,s,this.time,.82);x=hold.x;y=hold.y;speed=RULES.cruiseSpeed;if(target&&distance(s,target)<680){x=target.x;y=target.y;speed=stats.speed;}}
  ...
  if(this.fortressless){const a=s.phase+s.side*Math.PI;x=WORLD.width/2+Math.cos(a)*180;y=WORLD.height/2+Math.sin(a)*180;speed=stats.speed;}
 ...
 step(dt,input={}){
  ...
  if(this.time>=this.nextRecall&&this.time<RULES.recallCutoff)this.beginRecall();
  if(this.decisionPending&&this.time>=RULES.firstRecall+(this.orderNumber-1)*RULES.cycle+RULES.orderWindow-.2)this.setFallback();
  ...
  for(const s of this.ships){if(!s.alive)continue;s.cooldown-=dt;s.flash=Math.max(0,s.flash-dt);if(s.kind!=='artillery'&&s.kind!=='cargo'&&this.time-s.lastHit>=5)s.shield=2;
  ...
  this.checkCharge();
  const blue=this.living(0).length,red=this.living(1).length;
  if(this.time>=RULES.battleDuration)this.finish(blue===red?'draw':blue>red?'won':'lost',blue===red?'TIME · FLEETS TIED':'TIME · FLEET ADVANTAGE');
 ...
 static restore(d){
  ...
  const legacy=d?.version>=10&&d.version<=20,cargoSave=d?.version===21,modern=d?.version===22||d?.version===23||d?.version===24;
  if((!legacy&&!cargoSave&&!modern)||(legacy&&(d.ships?.length!==354||d.formations?.length!==20))||(cargoSave&&(d.ships?.length!==418||d.formations?.length!==28))||(modern&&(!Array.isArray(d.ships)||d.ships.length<402||d.ships.length>638||d.formations?.length!==30))||!Number.isFinite(d.time)||d.time<0||!Number.isInteger(d.playerId)||d.playerId<0||d.playerId>=d.ships.length)throw new Error('Saved battle belongs to a different version.');
  ...
  const b=Object.assign(Object.create(Battle.prototype),d);if(b.version===21){b.launchQueue=[];b.chargeAt=b.phase==='final'?b.time:null;b.addCavalryWings();b.version=22;}b.version=24;for(const s of b.ships)if(!Number.isFinite(s.x)||!Number.isFinite(s.y)||!Number.isFinite(s.hp)||!TYPES[s.kind])throw new Error('Invalid saved ship.');b.events=[];b.updateGroups();b.updateVision();return b;
 }
}
